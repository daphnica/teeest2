...
public class MusicPlayer extends Activity implements OnItemClickListener,CallbackDeleteEntity.OnMethodCallback{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Toast.makeText(getApplicationContext(), config.lastEnter, Toast.LENGTH_LONG).show();
		updateTime = new Date();
...
//		mMusicAdapter = new MusicAdapter(getApplicationContext(), null,mReceiver);
...
//		mMusicUtil.refreshData(this, mReceiver);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if(item.getTitle().equals(mMusicUtil.stringFormat(this,R.string.download))){
			startService(new Intent(getApplicationContext(),
					DownloadService.class));
		}else if(item.getTitle().equals(mMusicUtil.stringFormat(this,R.string.delete))){
			nBtnNo = 1;
			mMusicUtil.refreshData(getApplicationContext(), mReceiver);
		}else if(item.getTitle().equals(mMusicUtil.stringFormat(this,R.string.back))){
			nBtnNo = 2;
			mMusicUtil.refreshData(getApplicationContext(), mReceiver);
		}else if(item.getTitle().equals(mMusicUtil.stringFormat(this,R.string.deleteComfirm))){
			nBtnNo = 3;
			String [] delItem = sSumDeletItem.substring(0,sSumDeletItem.length()-1).split(","); 
			//文件物理删除
			...
			mMusicUtil.refreshData(getApplicationContext(), mReceiver);
		}else if(item.getTitle().equals(mMusicUtil.stringFormat(this,R.string.lan))){
			setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
			nMenuId=2;
			nLanorpor=2;
		}else if(item.getTitle().equals(mMusicUtil.stringFormat(this,R.string.por))){
...
////				dialog.show();
//			}else if(action.equals(Intent.ACTION_MEDIA_SCANNER_FINISHED)||action.equals(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE)){
...
//				switch(nBtnNo){
//				case 0 :
//					mMusicAdapter.initAdapter(context,null);
//					mMusicAdapter.notifyDataSetChanged();
//					break;
//				case 1 : 
//					mMusicAdapter.initAdapter(context,"isDelete");
//					mMusicAdapter.notifyDataSetChanged();
//					nMenuId=1;
//					break;
//				case 2 :
...
//	}
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		outState.putString("BreakTime","崩溃时间为:"+year+"-"+month+"-"+day
				+" "+hour+":"+min+":"+sec);
